...
(function () {
    ...
    var PROFANITY = [
        // English
        'fuck', 'fucking', 'fuk', 'shit', 'bitch', 'bastard', 'asshole', 'dick', 'pussy',
        'cunt', 'slut', 'whore', 'fag', 'faggot', 'nigger', 'motherfucker', 'cock', 'jerk',
        'damn', 'crap', 'retard', 'idiot', 'stupid',
        // Vietnamese (common vulgar terms / insults)
        'đụ', 'địt', 'đéo', 'lồn', 'cặc', 'buồi', 'đĩ', 'điếm', 'đm', 'đmm', 'vcl', 'vl',
        'cc', 'cứt', 'đù', 'đù má', 'đm', 'đcm', 'clm', 'cmm', 'cmn', 'súc vật', 'chó chết',
        'ngu', 'óc chó', 'mẹ mày', 'con mẹ', 'thằng chó', 'đồ ngu', 'khốn nạn', 'mất dạy',
    ];
    ...
    function findPII(text) {
        ...
        if (/\b[a-z0-9-]{2,}\.(com|net|org|vn|io|xyz|info|me|link|shop|app|co|tv)\b/i.test(t)) return 'liên kết';
    ...
    function check(text, opts) {
        ...
        if (raw.length > max) return { ok: false, reason: 'Nội dung quá dài (tối đa ' + max + ' ký tự).' };
        // simple spam guard: a single character repeated many times
        if (/(.)\1{9,}/.test(raw)) return { ok: false, reason: 'Tin nhắn nhìn giống spam, hãy viết lại nhé.' };
        if (hasProfanity(raw)) return { ok: false, reason: 'Tin nhắn có từ ngữ không phù hợp. Hãy viết lại lịch sự nhé! 😊' };
        var pii = findPII(raw);
        if (pii) return { ok: false, reason: 'Để an toàn, đừng chia sẻ ' + pii + ' nhé. Tin nhắn chưa được gửi.' };
        return { ok: true };
    }
    ...
    function checkName(name, opts) {
        ...
        if (hasProfanity(t)) return { ok: false, reason: 'Tên có từ ngữ không phù hợp, hãy chọn tên khác nhé.' };
        if (findPII(t)) return { ok: false, reason: 'Tên không được chứa email, số điện thoại hay liên kết.' };
        return { ok: true };
    }
