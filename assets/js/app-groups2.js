...
(function () {
    ...
    async function challengeScheduled(myGroupId, targetGroupName, scheduledAtISO, windowMin) {
        ...
        try {
            ...
            return {
                error: missing
                    ? 'Tính năng đặt lịch chưa sẵn sàng - quản trị viên cần chạy migration "group_battle_scheduling.sql".'
                    : 'Không thể gửi thách đấu lúc này.',
            };
        }
    }
        ...
        try {
            ...
            for (let i = 0; i < n; i++) {
                rows.push({
                    ...
                    user_a_id: A[i].user_id,
                    username_a: A[i].username,
                    user_b_id: B[i].user_id,
                    username_b: B[i].username,
                });
            }
            ...
            return {
                error: missing
                    ? 'Cần chạy migration "group_battle_scheduling.sql" trước.'
                    : 'Không thể nhận lời thách đấu lúc này.',
            };
        }
    }
    ...
    async function finalizeExpired(battle, pairs) {
        ...
        const windowEnd =
            new Date(battle.scheduled_at).getTime() + (battle.window_min || 30) * 60000;
        if (Date.now() < windowEnd) return null;
        try {
            let duels = [];
            try {
                duels = await window.Groups.getBattleDuels(battle.id);
            } catch (e) {}
            ...
            const winnerGroupId =
                winsA > winsB ? battle.group_a_id : winsB > winsA ? battle.group_b_id : null;
            await client
                .from('group_battles')
                .update({
                    status: 'finished',
            ...
            console.error('finalizeExpired failed:', e);
            return null;
        }
    }
    ...
    async function notifyOwner(myProfile, group, text) {
        try {
            if (
                ...
                !group ||
                !group.owner_id ||
                group.owner_id === myProfile.id ||
                !window.Inbox
            )
                return;
            await window.Inbox.sendDirectMessageToId(
                myProfile,
                group.owner_id,
                group.owner_username || 'Chủ nhóm',
                text
            );
        } catch (e) {
    ...
    async function sendChallengeLetter(myGroupId, targetGroupName, myProfile) {
        ...
        try {
            ...
            if (
                (existing || []).some(
                    (b) =>
                        b.challenge_kind === 'letter' &&
                        (b.group_a_id === target.id || b.group_b_id === target.id)
                )
            ) {
                return {
                    error: 'Bạn đang có một lời thách đấu với group này. Vào "Lịch thi đấu" để tiếp tục hoặc hủy nó rồi gửi lại.',
                };
            }
            const { data, error } = await client
                .from('group_battles')
            ...
            try {
                ...
                if (g)
                    await client
                        .from('groups')
                        .update({ battles_initiated: (g.battles_initiated || 0) + 1 })
                        .eq('id', myGroupId);
            } catch (e) {
            ...
            await notifyOwner(
                myProfile,
                target,
                '⚔️ Group của bạn nhận được một lời thách đấu mới! Vào Group → Lịch thi đấu để phản hồi.'
            );
            return { data };
        } catch (e) {
            console.error('sendChallengeLetter failed:', e);
            const missing = /invite_accepted|challenge_kind|column|schema cache/i.test(
                e.message || ''
            );
            return {
                error: missing
                    ? 'Tính năng thách đấu mới chưa sẵn sàng - quản trị viên cần chạy các migration "group_battle_upgrades.sql" và "group_battle_realtime.sql".'
                    : 'Không thể gửi thư thách đấu lúc này.',
            };
        }
    }
    ...
    async function withdrawBattle(battleId) {
        ...
        try {
            ...
            if (!data || data.length === 0) {
                return {
                    error: 'Không thu hồi được — trận đã bắt đầu, hoặc quản trị viên cần chạy migration "group_battle_delete_policy.sql".',
                };
            }
            return {};
        } catch (e) {
            return { error: 'Không thể hủy lúc này.' };
        }
    }
    ...
    async function forfeitBattle(battle, myGroupId, myProfile) {
        ...
        try {
            ...
            return {
                error: missing
                    ? 'Cần chạy migration "group_battle_realtime.sql" trước.'
                    : 'Không thể bỏ cuộc lúc này.',
            };
        }
    }
    ...
    async function syncScheduledBattle(battle) {
        ...
        try {
            ...
            /* live sync is best-effort - never break the board */
        }
    }
    ...
    async function postBattleMarquee(userId, username, message) {
        try {
            if (window.ActivityFeed && message) {
                await window.ActivityFeed.postEvent(
                    'group_battle',
                    userId || null,
                    username || 'Trọng tài',
                    message
        ...
        try {
            ...
            if (a)
                await notifyOwner(
                    myProfile,
                    a,
                    '✅ Đối thủ đã NHẬN LỜI thách đấu. Vào Lịch thi đấu để đặt ngày giờ và mức cược.'
                );
            return { data };
        } catch (e) {
            ...
            return {
                error: missing
                    ? 'Cần chạy migration "group_battle_upgrades.sql" trước.'
                    : 'Không thể nhận lời lúc này.',
            };
        }
    }
    ...
    async function declineInvite(battleId) {
        ...
        try {
            ...
            if (!data || data.length === 0) {
                return {
                    error: 'Không xóa được lời mời — quản trị viên cần chạy migration "group_battle_delete_policy.sql".',
                };
            }
            return {};
        } catch (e) {
            return { error: 'Không thể từ chối lúc này.' };
        }
    }
    ...
    async function setSchedule(battle, scheduledAtISO, windowMin, wagerXp, myProfile) {
        ...
        try {
            ...
            if (!isFirst) {
                if (changeCount >= 3)
                    return { error: 'Đã đổi lịch tối đa 3 lần - không thể đổi thêm.' };
                changeCount += 1;
            }
            const { data, error } = await client
                .from('group_battles')
                .update({
                    ...
                    schedule_change_count: changeCount,
                })
                .eq('id', battle.id)
                .select()
                .single();
            ...
            if (b)
                await notifyOwner(
                    myProfile,
                    b,
                    `📅 Đối thủ ${isFirst ? 'đã gửi' : 'vừa đổi'} lịch thách đấu & mức cược. Vào Lịch thi đấu để duyệt.`
                );
            return { data };
        } catch (e) {
        ...
        try {
            ...
            if (a)
                await notifyOwner(
                    myProfile,
                    a,
                    '🤝 Đối thủ đã CHẤP THUẬN lịch & mức cược. Trận đấu đã được chốt!'
                );
            return { data };
        } catch (e) {
            return { error: 'Không thể duyệt lịch lúc này.' };
        }
    }
    ...
    async function ensurePairsAndActivate(battle) {
        ...
        try {
            ...
            const onlineFirst = (arr) =>
                shuffle((arr || []).filter(isMemberOnline)).concat(
                    shuffle((arr || []).filter((m) => !isMemberOnline(m)))
                );
            const A = onlineFirst(membersA),
                B = onlineFirst(membersB);
            const n = Math.min(A.length, B.length);
            ...
            for (let i = 0; i < n; i++) {
                const a = A[i],
                    b = B[i];
                const aOn = isMemberOnline(a),
                    bOn = isMemberOnline(b);
            ...
            console.error('ensurePairsAndActivate failed:', e);
            return { error: e.message };
        }
    }
    ...
    async function finalizeScheduledBattle(battleId) {
        if (!client || !battleId) return null;
        try {
            const { error } = await client.rpc('finalize_scheduled_group_battle', {
            ...
            const rpcMissing =
                /finalize_scheduled_group_battle|does not exist|schema cache|function/i.test(
                    e.message || ''
                );
            if (rpcMissing) {
...
Object.assign(DuoClone.prototype, {
    ...
    chatAvatarHtml(username) {
        ...
        if (!this._chatAvatarHydrateQueued) {
            ...
            setTimeout(() => {
                this._chatAvatarHydrateQueued = false;
                this.hydrateChatAvatars();
            }, 60);
        }
    ...
    async hydrateChatAvatars() {
        try {
            ...
            const unknown = [...new Set(pending.map((el) => el.dataset.cavatar))].filter(
                (u) => !cache.has(u)
            );
            if (unknown.length && window.SupabaseClient && window.SupabaseClient.client) {
                ...
                unknown.forEach((u) => {
                    if (!cache.has(u)) cache.set(u, null);
                });
            }
            pending.forEach((el) => {
            ...
            /* avatars are decoration - never break chat */
        }
    },
    ...
    async openGroupActions(groupId, name) {
        ...
        try {
            mine =
                this.state.profile && window.Groups
                    ? await window.Groups.getMyGroup(this.state.profile.id)
                    : null;
        } catch (e) {}
        const isMyGroup = mine && mine.group && mine.group.id === groupId;
        const iAmAdmin =
            mine && mine.membership && ['owner', 'admin'].includes(mine.membership.role);
        ...
        overlay.querySelectorAll('.ga-btn').forEach((btn) =>
            btn.addEventListener('click', async () => {
                ...
                if (ga === 'info') this.renderGroupInfoCard(groupId);
                    ...
                    alert(
                        result.error
                            ? result.error
                            : 'Đã gửi yêu cầu tham gia! Chờ Chủ nhóm duyệt nhé.'
                    );
                } else if (ga === 'leave') {
                    ...
                    this.showConfirmDialog(
                        `Gửi thư thách đấu tới group "${name}"?`,
                        async () => {
                            const r = await window.GroupBattleSchedule.sendChallengeLetter(
                                myGroupId,
                                name,
                                this.state.profile
                            );
    ...
    async renderGroupInfoCard(groupId) {
        ...
        if (!g) {
            this.showBriefToast('Không tải được thông tin group.');
            this.renderGroupsMenu();
            return;
        }
        ...
        document
            .getElementById('group-info-back')
    ...
    renderCountdowns() {
        ...
        if (!nodes.length && !document.getElementById('battle-timer-widget')) {
            this.stopBattleTick();
            return;
        }
        nodes.forEach((el) => {
            ...
            if (left <= 0) {
                ...
                if (el.hasAttribute('data-countdown-refresh') && !el._firedRefresh) {
                    ...
                    if (this._battleBoardGroupId)
                        setTimeout(
                            () => this.renderBattleScheduleBoard(this._battleBoardGroupId),
                            800
                        );
                }
                return;
    ...
    startBattleBoardLive(viewGroupId) {
        ...
        const reload = () => {
            ...
            if (ae && ae.classList && ae.classList.contains('bb-chat-input')) {
                this._pendingBoardReload = true;
                return;
            }
            clearTimeout(this._boardReloadT);
            this._boardReloadT = setTimeout(() => this.renderBattleScheduleBoard(viewGroupId), 500);
        };
        this._battleBoardChannel = sb
            .channel('battle-board:' + viewGroupId)
            .on(
                'postgres_changes',
                {
                    event: '*',
                    schema: 'public',
                    table: 'group_battles',
                    filter: 'group_a_id=eq.' + viewGroupId,
                },
                reload
            )
            .on(
                'postgres_changes',
                {
                    ...
                    schema: 'public',
                    table: 'group_battles',
                    filter: 'group_b_id=eq.' + viewGroupId,
                },
                reload
            )
            .on(
                'postgres_changes',
                { event: '*', schema: 'public', table: 'group_battle_pairs' },
                reload
            )
            .on(
                'postgres_changes',
                { event: 'INSERT', schema: 'public', table: 'group_battle_chat' },
                (p) => this.onBattleChatInsert(p.new)
            )
            .subscribe();
    },
    stopBattleBoardLive() {
        const sb = window.SupabaseClient && window.SupabaseClient.client;
        if (this._battleBoardChannel && sb) {
            try {
                sb.removeChannel(this._battleBoardChannel);
            } catch (e) {}
        }
    ...
    onBattleChatInsert(msg) {
        ...
        if (!list) return;
        const empty = list.querySelector('.bb-chat-empty');
    ...
    async refreshBattleTimerWidget() {
        try {
            if (!this._battleTimerPoll)
                this._battleTimerPoll = setInterval(() => this.refreshBattleTimerWidget(), 60000);
            if (!this.state.profile || !window.Groups || !window.GroupBattleSchedule)
                return this.hideBattleTimer();
    ...
    showBattleTimer(myGroupId, kind, target) {
        ...
        if (!w) {
            ...
            w.addEventListener('click', () => {
                if (this._battleTimerGroupId)
                    this.renderBattleScheduleBoard(this._battleTimerGroupId);
            });
        }
    ...
    renderChallengeSearch(myGroupId) {
        ...
        this.ui.checkBtn.classList.remove('active');
        document
            .getElementById('chal-back')
            .addEventListener('click', () => this.renderBattleScheduleBoard(myGroupId));
        ...
        document.getElementById('chal-send').addEventListener('click', async () => {
            ...
            if (!target) {
                err.innerText = 'Nhập tên group đối thủ.';
                return;
            }
            const btn = document.getElementById('chal-send');
            btn.disabled = true;
            const r = await window.GroupBattleSchedule.sendChallengeLetter(
                myGroupId,
                target,
    ...
    renderScheduleEditor(battle, myGroupId) {
        ...
        const base = battle.scheduled_at
            ? new Date(battle.scheduled_at)
        ...
        const opt = (v, label) =>
            `<option value="${v}" ${win === v ? 'selected' : ''}>${label}</option>`;
        this.ui.container.innerHTML = `
            <div class="welcome-screen">
        ...
        document.getElementById('sched-send').addEventListener('click', async () => {
            ...
            const wager = Math.max(
                0,
                parseInt(document.getElementById('sched-wager').value, 10) || 0
            );
            const at = new Date(when);
            if (!when || isNaN(at.getTime()) || at.getTime() < Date.now() + 5 * 60000) {
                alert('Chọn thời gian ít nhất 5 phút sau hiện tại.');
                return;
            }
            const btn = document.getElementById('sched-send');
            btn.disabled = true;
            const r = await window.GroupBattleSchedule.setSchedule(
                battle,
                at.toISOString(),
                windowMin,
                wager,
    ...
    async renderBattleScheduleBoard(groupId) {
        ...
        const mine = this.state.profile
            ? await window.Groups.getMyGroup(this.state.profile.id).catch(() => null)
            : null;
        const myGroupId = mine && mine.group ? mine.group.id : null;
        const iAmAdmin = !!(
            mine &&
            mine.membership &&
            ['owner', 'admin'].includes(mine.membership.role)
        );
        ...
        const nameOf = async (id) => {
            ...
            const g = await window.Groups.getGroupById(id).catch(() => null);
            nameCache[id] = g ? g.name : '?';
            return nameCache[id];
        };
        ...
        for (const b of battles) {
            if (
                ...
                b.schedule_approved &&
                b.scheduled_at &&
                Date.now() >= new Date(b.scheduled_at).getTime()
            ) {
                const r = await S.ensurePairsAndActivate(b);
                if (r && r.pairs > 0) {
                    const na = await nameOf(b.group_a_id),
                        nb = await nameOf(b.group_b_id);
                    this.showBattleAlert(
                        '⚔️ ĐẾN GIỜ! Trận đấu đã bắt đầu — vào trận ngay!',
                        'live'
                    );
                    await S.postBattleMarquee(
                        this.state.profile && this.state.profile.id,
                        this.state.profile && this.state.profile.username,
                        `⚔️ TRẬN ĐẤU BẮT ĐẦU: ${na} 🆚 ${nb}! (${r.pairs} cặp)`
                    );
                }
            }
        }
        ...
        for (const b of battles) {
            if (b.status === 'active') await S.syncScheduledBattle(b);
        }
        battles = await S.getBattlesFor(viewGroupId);
        ...
        let cards = '';
        for (const b of battles)
            cards += await this.renderBattleCard(b, myGroupId, iAmAdmin, nameOf);
        let finishedCards = '';
        for (const b of finished)
            finishedCards += await this.renderFinishedBattleCard(b, myGroupId, nameOf);
        ...
        document.getElementById('sched-board-back').addEventListener('click', () => {
            this.stopBattleBoardLive();
            this.renderGroupsMenu();
        });
        const searchBtn = document.getElementById('chal-search-btn');
        if (searchBtn)
            searchBtn.addEventListener('click', () => this.renderChallengeSearch(myGroupId));
        this.wireBattleBoard(viewGroupId, battles);
        this.ui.container.querySelectorAll('.bb-chat-list').forEach((l) => {
    ...
    async announceFinishedBattles(finished, myGroupId, nameOf) {
        ...
        for (const b of finished || []) {
            ...
            const na = await nameOf(b.group_a_id),
                nb = await nameOf(b.group_b_id);
            const hi = Math.max(b.group_a_wins || 0, b.group_b_wins || 0),
                lo = Math.min(b.group_a_wins || 0, b.group_b_wins || 0);
            const resultTxt = !b.winner_group_id
                ? `HÒA ${b.group_a_wins || 0}-${b.group_b_wins || 0}`
                : `${this.escapeHtml(b.winner_group_id === b.group_a_id ? na : nb)} THẮNG ${hi}-${lo}`;
            await window.GroupBattleSchedule.postBattleMarquee(
                this.state.profile && this.state.profile.id,
                this.state.profile && this.state.profile.username,
                `🏁 KẾT THÚC: ${na} 🆚 ${nb} — ${resultTxt}${b.forfeited_by_group_id ? ' (bỏ cuộc)' : ''}`
            );
            if (myGroupId && (b.group_a_id === myGroupId || b.group_b_id === myGroupId)) {
                const iWon = b.winner_group_id === myGroupId;
                this.showBattleAlert(
                    `🏁 Trận đấu kết thúc! ${!b.winner_group_id ? '🤝 Hai group hòa nhau!' : iWon ? '🏆 GROUP BẠN CHIẾN THẮNG!' : '💪 Group bạn đã thua trận này.'}`,
                    iWon ? 'gold' : !b.winner_group_id ? 'info' : 'danger'
                );
            }
        }
    },
    ...
    async renderBattleCard(b, myGroupId, iAmAdmin, nameOf) {
        const nameA = await nameOf(b.group_a_id),
            nameB = await nameOf(b.group_b_id);
        ...
        const timeLabel = at
            ? `${at.toLocaleDateString('vi-VN')} ${String(at.getHours()).padStart(2, '0')}:${String(at.getMinutes()).padStart(2, '0')}`
            : '';
        const windowEnd = at ? at.getTime() + (b.window_min || 30) * 60000 : 0;
        const inWindow = at && Date.now() >= at.getTime() && Date.now() < windowEnd;
        const wagerLine =
            (b.wager_xp || 0) > 0
                ? `<div class="sched-card-time">💰 Cược: <b>${b.wager_xp}</b> EXP group</div>`
                : '';
        let body = '';

        if (b.status === 'pending' && !b.invite_accepted) {
            if (adminB)
                body = `<p style="color:#777; font-size:13px;">✉️ Group bạn nhận được thư thách đấu.</p>
                ...
                body = `<p style="color:#999; font-size:13px;">✉️ Đã gửi thư — chờ ${this.escapeHtml(nameB)} nhận lời.</p>
        ...
        if (at) {
            const tag = b.schedule_approved
                ? inWindow
                    ? '· <b style="color:#58a700">ĐANG DIỄN RA</b>'
                    : Date.now() >= windowEnd
                      ? '· đang tổng kết'
                      : '· ✅ đã chốt'
                : '· ⏳ chờ duyệt';
        ...
        if (!b.schedule_approved) {
            ...
            if (inWindow)
                body += `<div class="bb-countdown-wrap live">⚔️ Thời gian thi đấu còn <span class="bb-countdown" data-countdown-to="${windowEnd}" data-zero-text="HẾT GIỜ" data-countdown-refresh="1">--:--</span></div>`;
            body += this.renderPairRows(b, pairs, inWindow, windowEnd);
            if ((adminA || adminB) && b.status === 'active')
                body += `<button class="btn-secondary bb-forfeit" data-bid="${b.id}" style="padding:8px 16px; margin-top:8px; color:var(--duo-red);">🏳️ Bỏ cuộc (xử thua)</button>`;
        ...
        if ((adminA || adminB) && b.status === 'pending')
            body += `<button class="btn-secondary bb-withdraw" data-bid="${b.id}" style="padding:7px 14px; margin-top:6px; font-size:12px; color:#999;">❌ Hủy trận</button>`;
    ...
    renderPairRows(b, pairs, inWindow, windowEnd) {
        if (!pairs || !pairs.length)
            return `<p style="color:#999; font-size:13px;">Chưa có cặp đấu.</p>`;
        const rows = pairs
            .map((p) => {
                ...
                const status = p.winner
                    ? p.winner === 'draw'
                        ? '🤝 Hòa'
                        : `🏆 ${this.escapeHtml(p.winner === 'a' ? p.username_a : p.username_b)}`
                    : inWindow
                      ? `${p.joined_a_at ? '🟢' : '⚪'} vs ${p.joined_b_at ? '🟢' : '⚪'}`
                      : '⏳';
                const cd =
                    inWindow && !p.winner
                        ? `<span class="bb-pair-cd" data-countdown-to="${windowEnd}" data-zero-text="⌛">--:--</span>`
                        : '';
                const joinBtn =
                    inWindow && !p.winner && (meA || meB) && !(meA ? p.joined_a_at : p.joined_b_at)
                ...
                const duelBtn =
                    ...
                    !p.winner &&
                    (meA || meB) &&
                    (meA ? p.joined_a_at : p.joined_b_at) &&
                    (meA ? p.joined_b_at : p.joined_a_at)
    ...
    async renderBattleChat(b) {
        ...
        const list =
            (msgs || [])
                ...
                .join('') ||
            `<div class="bb-chat-empty">Chưa có tin nhắn. Trao đổi lịch đấu với chủ group đối thủ ở đây.</div>`;
    ...
    async renderFinishedBattleCard(b, myGroupId, nameOf) {
        ...
        const pairs = await window.GroupBattleSchedule.getPairs(b.id);
        let outcome, kind;
        if (!b.winner_group_id) {
            outcome = '🤝 Hai group HÒA';
            kind = 'info';
        } else {
            ...
            .map((p) => {
                const w =
                    p.winner === 'draw'
                        ? '🤝 Hòa'
                        : p.winner === 'a'
                          ? `🏆 ${this.escapeHtml(p.username_a)}`
                          : p.winner === 'b'
                            ? `🏆 ${this.escapeHtml(p.username_b)}`
                            : '—';
                return `<div class="bb-result-pair"><span>${this.escapeHtml(p.username_a)} ⚔️ ${this.escapeHtml(p.username_b)}</span><span>${w}</span></div>`;
            })
            .join('');
        const wagerNote =
            (b.wager_xp || 0) > 0 && b.winner_group_id
                ? `<div class="bb-result-line">💰 Group thắng nhận <b>${b.wager_xp}</b> EXP cược.</div>`
                : '';
        const stealNote = b.winner_group_id
            ? `<div class="bb-result-line">🗡️ Mỗi thành viên group thắng có vào trận đã cướp 10% XP của đối thủ ghép cặp.</div>`
            : '';
    ...
    wireBattleBoard(viewGroupId, battles) {
        ...
        const on = (sel, fn) =>
            this.ui.container
                ...
                .forEach((btn) => btn.addEventListener('click', () => fn(btn)));
        const namesOf = async (b) => [
            await window.Groups.getGroupById(b.group_a_id).catch(() => null),
            await window.Groups.getGroupById(b.group_b_id).catch(() => null),
        ];

        on('.bb-accept-letter', async (btn) => {
            btn.disabled = true;
            const r = await window.GroupBattleSchedule.acceptInvite(
                findB(btn.dataset.bid),
                this.state.profile
            );
            if (r.error) {
                alert(r.error);
            ...
            this.showBattleAlert(
                '✅ Đã NHẬN LỜI thách đấu! Chờ đối thủ đặt lịch & mức cược.',
                'success'
            );
            reload();
        });
        on('.bb-decline-letter', (btn) =>
            this.showConfirmDialog(
                'Từ chối lời thách đấu này?',
                async () => {
        ...
        on('.bb-set-schedule', (btn) =>
            this.renderScheduleEditor(findB(btn.dataset.bid), viewGroupId)
        );
        on('.bb-approve', async (btn) => {
            ...
            const at = b.scheduled_at ? new Date(b.scheduled_at) : null;
            const t = at
                ? `${String(at.getHours()).padStart(2, '0')}:${String(at.getMinutes()).padStart(2, '0')} ${at.toLocaleDateString('vi-VN')}`
                : '';
            await window.GroupBattleSchedule.postBattleMarquee(
                this.state.profile.id,
                this.state.profile.username,
                `🔒 CHỐT TRẬN: ${ga ? ga.name : '?'} 🆚 ${gb ? gb.name : '?'} lúc ${t}${(b.wager_xp || 0) > 0 ? ` · cược ${b.wager_xp} EXP` : ''}!`
            );
            reload();
        });
        on('.bb-withdraw', (btn) =>
            this.showConfirmDialog(
                'Hủy trận đấu này? (chỉ được khi trận chưa bắt đầu)',
                async () => {
        ...
        on('.bb-forfeit', (btn) =>
            this.showConfirmDialog(
                'Bỏ cuộc? Group bạn sẽ bị XỬ THUA trận này và mất điểm cược.',
                async () => {
                    const b = findB(btn.dataset.bid);
                    const r = await window.GroupBattleSchedule.forfeitBattle(
                        b,
                        viewGroupId,
                        this.state.profile
                    );
                    ...
                    const [ga, gb] = await namesOf(b);
                    const winner = viewGroupId === b.group_a_id ? gb : ga,
                        loser = viewGroupId === b.group_a_id ? ga : gb;
                    await window.GroupBattleSchedule.postBattleMarquee(
                        this.state.profile.id,
                        this.state.profile.username,
                        `🏳️ ${loser ? loser.name : '?'} BỎ CUỘC — ${winner ? winner.name : '?'} thắng!`
                    );
                    reload();
                },
                { okLabel: 'BỎ CUỘC' }
        ...
        on('.bb-chat-send', async (btn) => {
            ...
            const r = await window.GroupBattleSchedule.sendBattleChat(
                btn.dataset.bid,
                this.state.profile,
                input ? input.value : ''
            );
        ...
        on('.sched-join', async (btn) => {
            ...
            const p = (pairs || []).find((x) => x.id === btn.dataset.pid) || {
                id: btn.dataset.pid,
            };
        ...
        on('.sched-duel', async (btn) => {
            btn.disabled = true;
            const r = await this.sendGameDuelChallenge(
                btn.dataset.opponent,
                'lesson',
                btn.dataset.bid,
                btn.dataset.side
            );
    ...
    async checkScheduledBattleWindow() {
        try {
            ...
            for (const b of battles) {
                ...
                const pairs = await window.GroupBattleSchedule.getPairs(b.id);
                const minePair = pairs.find(
                    (p) =>
                        !p.winner &&
                        (p.user_a_id === this.state.profile.id ||
                            p.user_b_id === this.state.profile.id)
                );
                ...
                if (host) {
                    ...
                    banner.addEventListener('click', () =>
                        this.renderBattleScheduleBoard(mine.group.id)
                    );
                    host.prepend(banner);
                }
                return;
            ...
            /* purely additive - never break the dashboard */
        }
    },
});
